...
function getFirstChar(value) {
  if (Array.from(value).length > 0) return value.charAt(0);
  return '';
}
...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((soc, i) => soc + i.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < string.length; i += 1) {
    const char = string[i];
    if (legend.includes(char)) count += 1;
  }
  return count;
}
...
function isPalindrome(str) {
  ...
  let half = 0;
  let center = 0;
  half = Math.floor((newString.length + 1) / 2);
  if (newString.length % 2 === 0) center = 1;
  const start = newString.slice(0, half - 1);
  const end = newString.slice(half + center, newString.length);
  const reverse = end.split('').reverse().join('');
  if (start === reverse) return true;
  return false;
}
...
function findLongestWord(sentence) {
  ...
  const longestWord = arr.reduce((long, curr) => {
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverse = arr.map((el) => el.split('').reverse().join(''));
  return reverse.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < charArr.length; i += 1) {
    if (charArr[i] === 32) {
      result += ' ';
    } else if (charArr[i] === 33) {
      result += '!';
    } else if (charArr[i] === 63) {
      result += '?';
    } else if (charArr[i] >= 97 && charArr[i] <= 109) {
      result += String.fromCharCode(charArr[i] + 13);
    } else if (charArr[i] >= 110) {
      result += String.fromCharCode(charArr[i] - 13);
    } else if (charArr[i] >= 65 && charArr[i] <= 77) {
      result += String.fromCharCode(charArr[i] + 13);
    } else {
      result += String.fromCharCode(charArr[i] - 13);
    }
